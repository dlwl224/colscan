...
사용자 질문과 함께 제공된 URL 분석 결과 및 세부 특징 데이터를 바탕으로, 각 항목을 비전문가가 이해할 수 있게 왜 해당 URL이 위험하거나 안전한지 친절하고 상세하게 설명해주세요.
절대로 비유를 사용하지 말고, 사실 기반의 핵심만 전달해야 합니다.
너무 길지 않게, 각각의 특징별 설명을 짧은 문장으로 가독성있게 간결하게 작성해주세요.
[사용자 질문]
{user_query}
...
2. **핵심 요약 (Summary)**
   ...
   - **'서브도메인'과 같은 기술 용어가 나오면, 그것이 무엇인지 비전문가(일반인 사용자들이) 알 수 있게  짧은 단어로  풀어서 설명해주세요. (예: 주소의 세부 단계(서브도메인)가...)**

3. **상세 설명 (Details)**
   - 한 문장을 쉬운 말로 작성하고 그 길이가 너무 길지 않도록 주의해주세요. 문단 간격도 적절히 활용해주세요.
   - 요약 아래에, '세부 특징 데이터'를 근거로 들어 왜 그렇게 판단했는지 2~3가지 핵심 이유를 구체적으로 설명해주세요.
   - 각 이유를 번호를 붙여 명확하게 구분해주세요.
   -**설명에 URL의 실제 일부를 예시로 포함하여 사용자가 직관적으로 이해할 수 있도록 도와주세요. 
4. **사용자 행동 요령**
